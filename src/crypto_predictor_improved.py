...
# Constants
# Default to GPU-trained improved model directory
DEFAULT_MODEL_DIR = PROJECT_ROOT / 'data' / 'models_gpu_improved'
...
class ImprovedCryptoPredictor:
    def __init__(self, model_dir=None):
        ...
        Args:
            model_dir (str, optional): Directory containing the trained model files.
        ...
        self.verbose = True  # controls how much gets printed during prediction
    ...
    def load_model(self, model_dir):
        ...
        try:
            ...
            if not model_path.exists():
                ...
                model_path = model_dir / 'crypto_advanced_model.h5'
                if not model_path.exists():
            ...
            # Load scalers
            self.price_scaler = joblib.load(model_dir / 'price_scaler.pkl')
    ...
    def predict_next_days(self, df, days=3):
        ...
        # Suppress verbose feature logs during multi-day simulation
        original_verbose = getattr(self, 'verbose', True)
        self.verbose = False
...
def main():
    ...
    # Initialize predictor (loads from GPU-trained models by default)
    ...
    # Look for CSV data in data/raw_data first, then fall back to project root
    data_dir = PROJECT_ROOT / 'data' / 'raw_data'
    csv_files = list(data_dir.glob('coin_*.csv'))

    if not csv_files:
        # Fallback: look in project root (legacy layout)
        csv_files = list(PROJECT_ROOT.glob('coin_*.csv'))
        
    if not csv_files:
        print(f"❌ No cryptocurrency data files found in {data_dir} or project root")
    ...
    for csv_file in csv_files:
        print("\n" + "-" * 60)
        print(f"📊 Coin file: {csv_file.name}")
        df = load_crypto_data(csv_file)
        
        if df is None or df.empty:
            print(f"   ❌ Failed to load data from {csv_file.name}")
            continue
                
        try:
            if 'close' not in df.columns:
                print(f"   ❌ No price data found in {csv_file.name}")
                continue

            # Skip coins with invalid last price (NaN or effectively zero)
            current_price = df['close'].iloc[-1]
            if pd.isna(current_price) or abs(current_price) < 1e-6:
                print(f"   ⚠️ Skipping {csv_file.name} due to invalid current price ({current_price})")
                continue
